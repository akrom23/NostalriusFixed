...
struct npc_WindsorAI : public ScriptedAI
{
    ...
    // [Daemon] : Attention, on ne stocke pas de pointeurs ! Seulement les GUIDs !
    ...
    void Parlotte(Unit* Parleur, bool Crier, Unit* Target = nullptr)
    {
        (Crier == true) ? Parleur->MonsterYell(NOST_TEXT(ParlotteWindsor[IDSpeech]), 0, Target) : Parleur->MonsterSay(NOST_TEXT(ParlotteWindsor[IDSpeech]), 0, Target);
        IDSpeech++;
    }
        ...
        if (Begin == true)
        {
            if (m_creature->GetDistance2d(WindsorDeplacement[0].X, WindsorDeplacement[0].Y) < 2.0f)
            {
                ...
                if (Creature* Cheval = m_creature->FindNearestCreature(12581, 10.0f))
                {
                    ...
                    Cheval->ForcedDespawn(12000);
                    m_creature->SetFacingToObject(Cheval);
                    m_creature->MonsterSay("Yawww!");
                    m_creature->HandleEmote(EMOTE_ONESHOT_ATTACKUNARMED);
                }
                if (Creature* Rowe = m_creature->FindNearestCreature(17804, 100.0f))
                    Rowe->ForcedDespawn(12000);
            ...
            if (GreetPlayer)
            {
                if (Timer <= uiDiff)
                {
                    ...
                    if (pPlayer)
                    {
                        ...
                        m_creature->MonsterSay(sMessage, 0, 0);
        ...
        if (Timer < uiDiff)
        {
            ...
            switch (Tick)
            {
                ...
                case 1:
                    ...
                    if (Creature* Onyxia = m_creature->SummonCreature(1749, -9075.6f, 466.11f, 120.383f, 6.27f, TEMPSUMMON_TIMED_DESPAWN, 10 * IN_MILLISECONDS))
                    {
                        Onyxia->SetDisplayId(11686);        // Hack pour avoir le yell d'Onyxia
                        Onyxia->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        Onyxia->MonsterYellToZone(NOST_TEXT(ParlotteWindsor[IDSpeech]));
                        IDSpeech++;
                    }
                ...
                case 63:
                    if (Creature* Onyxia = m_creature->FindNearestCreature(1749, 150.0f))
                    {
                        ...
                        for (std::list<Creature*>::iterator itr = DragListe.begin(); itr != DragListe.end(); ++itr)
                        {
                            ...
                            if (!urand(0,2)) (*itr)->MonsterTextEmote("Onyxia's Elite Guard hisses.");
                            Var++;
                        }
                    }
                    Timer = 4000;
                ...
                case 64:
                    if (Creature* Onyxia = m_creature->FindNearestCreature(1749, 150.0f))
                        Onyxia->CastSpell(m_creature, 20465, false);
                    Timer = 1500;
                    break;
                case 65:
                    Parlotte(m_creature, false);
        ...
        if (Timer < uiDiff)
        {
            ...
            switch (Tick)
            {
                case 0:
                    ...
                    Timer = 1000 + sqrt((X * X) + (Y * Y)) / (m_creature->GetSpeed(MOVE_RUN) * 0.001f);
                    break;
                case 1:
                ...
                case 5:
                    if (Creature* Windsor = m_creature->SummonCreature(12580, -9148.395508f, 371.322174f, 90.543655f, 0.700131f, TEMPSUMMON_MANUAL_DESPAWN, 1.5 * HOUR * IN_MILLISECONDS))
                    {
                        if (playerGuid)
                        {
                            if (npc_WindsorAI* pWindsorAI = dynamic_cast<npc_WindsorAI*>(Windsor->AI()))
                                pWindsorAI->playerGUID = playerGuid;
                        }
                        Windsor->Mount(2410);
                        Windsor->SetWalk(false);
                        Windsor->SetSpeedRate(MOVE_RUN, 1.0f, true);
...
{
    if (pPlayer->GetQuestStatus(6402) == QUEST_STATUS_COMPLETE && pPlayer->GetQuestStatus(6403) != QUEST_STATUS_COMPLETE)
    {
        ...
        pPlayer->SEND_GOSSIP_MENU(9065, pCreature->GetGUID());
    }
    else
        ...
        case GOSSIP_ACTION_INFO_DEF:
            if (pPlayer->GetQuestStatus(6402) == QUEST_STATUS_COMPLETE)
                if (npc_RoweAI* pRoweEventAI = dynamic_cast<npc_RoweAI*>(pCreature->AI()))
                {
                    ...
                    pRoweEventAI->playerGuid = pPlayer->GetObjectGuid();
...
struct npc_bolvar_fordragonAI : public ScriptedAI
{
    npc_bolvar_fordragonAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        Reset();
    }
    ...
    { 
        ...
        else m_uiHighlordsJusticeTimer -= uiDiff;
